...
const Header =(props)=>{
    ...
    const {receivedData, closeNotificationAlert, authLogout, igVideoImg, handleFollowing, getUsersProfile, changeMainState, searchUsers, searchInfo} = context;
            ...
            <header id="header" className="main--header flex-row">
                <div className="header--inner flex-row">
                   ...
                   <div className="search--bar--container">
                       <input value={searchVal} onChange={(e) => setSearchVal(e.target.value)} type="text" className="search__input" aria-label="search bar" placeholder="Search" autoCapitalize="none" />
                            <span className="search__icon"><RiSearchLine /></span>
                           {
                               searchInfo?.loading ?
                               <span className="loading--search--box"><Loader
                            type="Puff"
                            color="#919191"
                            height={19}
                            width={19}
                            ...
                            /></span>
                              : searchVal ?
                               <span  onClick={()=> clearSearchBox()} className="clear--search--box"><TiDelete /></span>
                              : null
                           } 
                                            ...
                                            <ul className="noti--popup--ul flex-column">
                                                {
                                                    ...
                                                    searchInfo?.results?.map( (user, i) =>{
                                                        return(
                                                                <SearchItem key={user?.uid || i} user={user} browseUser={browseUser} />
                                                        )
                                                    })
                                                    :
                                                    <div className="empty--box flex-row"><h4>No Results found</h4></div>
                                                }
                    ...
                    <nav className="header--nav flex-row">
                                ...
                                <div style={{
                                 ...
                                 <div className="noti--popup--arrow"> </div>
                                    <div className="noti--popup--inner">
                                        ...
                                        : 
                                            <div className="empty--box flex-row"><h4>No notifications available right now</h4></div>
                                        }
                        ...
                        </ul>
                        {
                            openProf || openNoti || openSearchBox ?
                                <div className="hidden--backdrop" onClick={()=> {setProf(false); setNoti(false); setSeachBox(false)}}></div>
                            : null
                        }
                    </nav>
                </div>
